        ...
        <div className="flex flex-wrap -m-4">
          {
            products.map((product) => {
              const { _id, name, category, net_weight, price, image } = product;
              return (
                <Link to={{
                  pathname: `/products/${_id}`
                }} state={product} className="lg:w-[23%] md:w-1/2 p-4 w-full mb-4 cursor-pointer rounded-lg shadow ml-4">
                  ...
                  <div className="mt-4">
                    ...
                    <p className="mt-1 text-md font-semibold">{net_weight}</p>
                  </div>
                </Link>
              )
            })
